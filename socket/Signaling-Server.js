...
var port= config.get("app.port");
    ...
    function appendUser(socket) {
        ...
        if (!alreadyExist) {
            var options = {
                host: url,
                port: port,
                path: "/peer/save/" + socket.handshake.query.videochatid + "/" + socket.userid,
                method: 'GET'
            };
    ...
    function onConnection(socket) {
        ...
        if (config.get("enableScalableBroadcast")) {
        ...
        if (listOfUsers[params.userid]
            || ( listOfUsers[socket.handshake.query.videochatid]
            &&  listOfUsers[socket.handshake.query.videochatid].socket.handshake.query.userid==params.userid)) {
        ...
        socket.on('disconnect', function() {
            ...
            try {
                var options = {
                    host: url,
                    port: port,
                    path: "/peer/remove/"+socket.userid,
                    method: 'GET'
                };
                http.request(options, function(res) {
                    console.log('STATUS: ' + res.statusCode);
                ...
                pushLogs('disconnect remove from mongo!', e);
            }
            ...
            try {
                ...
                if (listOfUsers[socket.userid]) {
                    ...
                    if (socket.shiftModerationControlBeforeLeaving && firstUserSocket) {
                        firstUserSocket.emit('become-next-moderator', sessionid);
                    }
                }
            } catch (e) {
                pushLogs('disconnect', e);
...
if (config.get("enableLogs")) {
    enableLogs = config.get("enableLogs");
}
